"""Greedy two-adapter eval: boot vLLM once, run old + new on the same prompts.

Same generation recipe as eval_3way_vllm.py but skips the base condition (we
have it cached when we need it). Used to compare old v9 against the warm-start
adapter at T=0.0 on the full dev_frozen n=500.
"""
...
def main() -> None:
    ...
    print(f"[eval2] split={args.split_file.name}  n={len(rows)}  ids[:5]={ids[:5]}", flush=True)

    from vllm import LLM, SamplingParams
    from vllm.lora.request import LoRARequest

    print(f"[eval2] booting LLM (~5 min)...", flush=True)
    llm = LLM(
        model=args.model,
        dtype="bfloat16",
    ...
    print(f"[eval2] LLM ready.", flush=True)
    ...
    results = {}
    for label, adapter_path, lora_id in (
        ...
        results[label] = score
        print(
            f"[eval2] {label:14s}  acc={score['accuracy']:.4f}  trunc={score['trunc_rate']:.2f}  elapsed={elapsed:.1f}s",
            flush=True,
        )
    ...
    print(f"[eval2] wrote {args.output}", flush=True)
