"""Three-way eval (base / old adapter / new adapter) on a fixed n=50 slice.

Boots vLLM exactly once, then runs three generation passes — one per condition —
on the same prompts with greedy decoding (temperature=0). Scoring uses the same
Kaggle extraction (`extract_boxed_answer` + `answers_match`) as `run_eval.py`.
"""
...
def main() -> None:
    ...
    print(f"[eval3] split={args.split_file.name}  n={len(rows)}  ids[:5]={ids[:5]}", flush=True)

    from vllm import LLM, SamplingParams
    from vllm.lora.request import LoRARequest

    print(f"[eval3] booting LLM (~5 min)...", flush=True)
    llm = LLM(
        model=args.model,
        dtype="bfloat16",
    ...
    print(f"[eval3] LLM ready.", flush=True)
    ...
    results = {}
    for label, lora_req in (
        ("base", None),
        ("old_v9", LoRARequest("old_v9", 1, str(args.old_adapter))),
        ("new", LoRARequest("new", 2, str(args.new_adapter))),
    ):
        t0 = time.time()
        ...
        results[label] = score
        print(
            f"[eval3] {label:8s}  acc={score['accuracy']:.4f}  trunc={score['trunc_rate']:.2f}  "
    ...
    print(f"[eval3] wrote {args.output}", flush=True)
