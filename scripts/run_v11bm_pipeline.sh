#!/usr/bin/env bash
# v11bm full pipeline (Q8 single-variable test): stage1 -> stage2(warm-start
...
LOG=runs/train/v11bm_pipeline.log
SENTINEL=runs/train/v11bm_pipeline.DONE
rm -f "$SENTINEL"
exec > >(tee -a "$LOG") 2>&1
echo "================ v11bm pipeline start $(date -u +%FT%TZ) ================"

echo "[1/4] stage-1 reasoning LoRA (lora_v11bm)"
if [ -f runs/train/lora_v11bm/adapter_model.safetensors ]; then
  echo "  stage-1 already saved — skipping."
else
  python -m src.training.train_lora --config configs/train/lora_v11bm.yaml
fi

echo "[2/4] stage-2 warm-start expert LoRA, 300 steps (lora_v11bm_ws300)"
...
if [ -f runs/train/lora_v11bm_ws300/adapter_model.safetensors ]; then
  echo "  stage-2 already saved — skipping."
else
  python -m src.training.train_lora --config configs/train/lora_v11bm_ws300.yaml --max-steps 300 --smoke
fi
...
python runs/train/_convert_buggy.py \
  --src runs/train/lora_v11bm_ws300 \
  --dst runs/train/lora_v11bm_ws300_vllm
